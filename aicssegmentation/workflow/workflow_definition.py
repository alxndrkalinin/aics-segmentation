...
class PrebuiltWorkflowDefinition(WorkflowDefinition):
    ...
    def thumbnail_pre(self) -> np.ndarray:
        ...
        return np.squeeze(
            imread(
                Directories.get_assets_dir() / f"thumbnails/{self.name.lower()}_pre.png"
            )
        )
    ...
    def thumbnail_post(self) -> np.ndarray:
        ...
        return np.squeeze(
            imread(
                ...
                / f"thumbnails/{self.name.lower()}_post.png"
            )
        )
    ...
    def diagram_image(self) -> np.ndarray:
        ...
        """
        return np.squeeze(
            imread(Directories.get_assets_dir() / f"diagrams/{self.name.lower()}.png")
        )
