...
class TestWorkflowEngine:
    ...
    def setup_method(self):
        ...
        self._mock_workflow_config.get_available_workflows.return_value = (
            self.expected_workflow_names
        )
        self._mock_workflow_config.get_workflow_definition.side_effect = (
    ...
    def test_workflow_definitions(self):
        assert (
            self._workflow_engine.workflow_definitions
            == self.expected_workflow_definitions
        )
    ...
    def test_get_executable_workflow_from_config_file_null_path_fails(self):
        with pytest.raises(ArgumentNullError):
            self._workflow_engine.get_executable_workflow_from_config_file(
                None, np.ones((1, 1, 1))
            )
    ...
    def test_get_executable_workflow_from_config_file(self, path):
        workflow = self._workflow_engine.get_executable_workflow_from_config_file(
            path, np.ones((1, 1, 1))
        )
        assert isinstance(workflow, Workflow)
    ...
    def test_save_workflow_definition(self):
        ...
        # Assert
        self._mock_workflow_config.save_workflow_definition_as_json.assert_called_once_with(
            workflow_def, path
        )
